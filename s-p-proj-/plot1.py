...
colormap=["red","cyan","blue","black","purple","beige","gray","cyan","green","pink","brown","black","orange"]
c=0
fig, ax = plt.subplots()
...
for f in files:
	...
	#color = cmap(dos[:,c])[..., :3]
	if c%2==1:
		ax.scatter(k,e[:,c] - Eg,c=colormap[int(c/2)],s=(dos[:,c]+dos[:,c-1])*ps, alpha=0.5)  #sum j splitting of s or ptype orbitals
		c=c+1
	else:
		c=c+1
...
#Plce the line at high symmetric points
for i in range(4,Nhsp+3):
	ax.axvline(x = float(mylines[i]), color = 'black')

ax.text(-0.05,-3.45, r"$ \Gamma $",fontsize=16)
ax.text(0.55378,-3.45, 'T',fontsize=20)
ax.text(1.059819,-3.45, 'H2|H0',fontsize=20)
ax.text(1.6660,-3.45, 'L',fontsize=20)
ax.text(2.3046,-3.45, r"$\Gamma $",fontsize=16)
ax.text(2.85508,-3.45, 'S0|S2',fontsize=16)
ax.text(4.0747,-3.45, 'F2',fontsize=16)
ax.text(4.83,-3.45, r"$\Gamma $",fontsize=16)
